...
function addGUI() {
  GUI.add(SETTINGS.length, "value", SETTINGS.length.min, SETTINGS.length.max)
    .name("Length")
    .step(1)
    .onChange(reset);
  GUI.add(
    ...
    "value",
    SETTINGS.segmentLength.min,
    SETTINGS.segmentLength.max
  )
    .name("Segment Length")
    .step(1)
    .onChange(reset);
  GUI.add(
    ...
    "value",
    SETTINGS.strokeWidth.min,
    SETTINGS.strokeWidth.max
  )
    .name("Stroke Width")
    .step(1)
    .onChange(reset);
  ...
  draw() {
    ...
    for (let i = 0; i < this.x.length - 1; i++) {
      if (this.color === "rainbow")
        stroke(this.rainbowColors[i % this.rainbowColors.length]);
